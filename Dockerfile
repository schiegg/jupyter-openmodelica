...
RUN DEBIAN_FRONTEND=noninteractive apt-get install -y gnupg lsb-release wget
...
RUN wget -q https://build.openmodelica.org/apt/openmodelica.asc -O- | apt-key add -

# Update index (again)
RUN apt-get update

# Install minimal OpenModelica Components
RUN DEBIAN_FRONTEND=noninteractive apt-get install -y omc
...
RUN pip3 install -U git+https://github.com/OpenModelica/jupyter-openmodelica.git
